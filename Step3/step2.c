...
void getProcessInformation(char pid[],FILE *file)
{
    FILE *inFile;
    int i = 1;
    char line[200];
    ...
    else
    {
        ...
        while (fgets(line,30, inFile) != NULL)
        {
            if(line[0]=='N' && line[1]=='a'){
                (pi.processName) = line;
                fprintf(file,"%s",(pi.processName));  
            }
            if(line[0]=='S' && line[1]=='t'){
                (pi.processState) = line;
                fprintf(file,"%s",(pi.processState));
            }
            if(line[0]=='V' && line[1]=='m' && line[2]=='S' && line[3]=='i'){
                (pi.msi.vmSize)=line;
                fprintf(file,"%s",(pi.msi.vmSize));
            }
            if(line[0]=='V' && line[1]=='m' && line[2]=='D'){
                (pi.msi.vmData) = line;
                fprintf(file,"%s",(pi.msi.vmData));
            }
            if(line[0]=='V' && line[1]=='m' && line[2]=='S' && line[3]=='t'){
                (pi.msi.vmStk) = line;
                fprintf(file,"%s",(pi.msi.vmStk));
            }
            if(line[0]=='V' && line[1]=='m' && line[2]=='E'){
                (pi.msi.vmExe) = line;
                fprintf(file,"%s",(pi.msi.vmExe));
            }
            if(line[0]=='v'){
                (pi.cci.volunteers) = line;
                fprintf(file,"%s",(pi.cci.volunteers));
            }
            if(line[0]=='n'){
